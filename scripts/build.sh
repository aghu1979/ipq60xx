...
extract_luci_packages() {
    local config_file="$1"
    local output_file="$2"
    grep "CONFIG_PACKAGE_luci-app.*=y" "$config_file" | sed 's/CONFIG_PACKAGE_//g; s/=y//g' | sort > "$output_file"
}
...
compare_luci_packages() {
    ...
    comm -23 "$before_file" "$after_file" > "$missing_file"
...
build_firmware() {
    ...
    if [[ -f "${BASE_DIR}/scripts/diy.sh" ]]; then
        ...
        echo -e "${COLOR_CYAN}详细日志：${LOG_DIR}/${REPO_SHORT}-${CONFIG_LEVEL}-diy.log${COLOR_RESET}"
        
        print_step_result "DIY脚本执行完成"
    else
        echo -e "\n${COLOR_YELLOW}⚠️ 未找到自定义脚本: ${BASE_DIR}/scripts/diy.sh${COLOR_RESET}"
        echo -e "${COLOR_YELLOW}ℹ️ 跳过自定义脚本步骤${COLOR_RESET}"
        print_step_result "跳过DIY脚本执行"
    fi
    
    # 步骤6：编译固件
    print_step_title "步骤6：编译固件"
    log_info "🔥 开始编译固件..."
